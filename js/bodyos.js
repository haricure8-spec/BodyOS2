const BodyOS = (() => {
  ...
  function rankElements(counts){
    return Object.entries(counts).sort((a,b)=>b[1]-a[1] || ["木","火","土","金","水"].indexOf(a[0])-["木","火","土","金","水"].indexOf(b[0]));
  }
  function ugFromFive(counts){
    const entries=rankElements(counts);
    const max=entries[0];
  ...
  function blueprint(result){
    const counts=fiveElements(result.pillars);
    const rank=rankElements(counts);
    const strong=rank[0][0], weak=rank[rank.length-1][0];
    const db=window.BODYOS_CARE_DB?.elements || {};
    return {counts,strong,weak,strongData:db[strong],weakData:db[weak]};
  }
  function environment(date=new Date()){
    const term=Calendar.currentSolarTerm(date);
    const db=window.BODYOS_CARE_DB || {};
    const season=(term && db.termSeason?.[term.name]) || "春";
    return {term,season,data:db.seasons?.[season]};
  }
  ...
  function care(codes){
    ...
    if(combo) return {codes:clean,summary:combo.summary,care:combo.care,next:combo.next,focus:clean.flatMap(c=>window.UGDB[c]?.focus||[]).slice(0,3),theme:combo.theme};
    let care=[],next=[],focus=[],summary=[];
    clean.slice(0,2).forEach(c=>{const d=window.UGDB[c]; if(!d) return; care.push(...d.care); next.push(...d.next); focus.push(...d.focus); summary.push(d.summary);});
  ...
  function carePlan(result,date=new Date()){
    const bp=blueprint(result);
    const env=environment(date);
    const current=care(currentCodes(result));
    const labActive=checked("beforeTest").length>0 || checked("afterTest").length>0;
    const food=unique([...(env.data?.food?[env.data.food]:[]),...(bp.strongData?.food||[])]).slice(0,3);
    const life=unique(bp.strongData?.lifestyle||[]).slice(0,3);
    return {blueprint:bp,environment:env,current,labActive,food,life};
  }
  ...
  return {fiveElements,ugFromFive,blueprint,environment,carePlan,checked,currentCodes,care,patientLabel,found,changed};
})();
